...
public class RearrangedServerSimulation {
    ...
    public RearrangedServerSimulation(String dur){
    ...
    public void initialArrival(){
        el.addEvent(new Event(3,clock,genArrivalTime(), EventType.ARRIVAL));     //create new arrival event and add to event list
        calculateInterTime();
    }
    ...
    private Server nextServer(int server){                                  //method returns the next Server given the value of the current server
        if(server==3){                                                      //this method is useful when the departure of a customer from current server has to go to the next server
            return s1;
        }else if(server==1){
            return s2;
        }else{
    ...
    private void eventArrive(Event e){                                      //process to take when the next Event from Event List is an ARRIVAL EVENT
        ...
        if(e.getServer()==3){                                               //if this is server 3, we also want to create the next interarrival time
    ...
    private void eventDepart(Event e){                                      //process to take if this is a DEPARTURE EVENT
        ...
        if(e.getServer()==3 || e.getServer()==1){                                                   
            if(nextServer.isBusy()){                                        //if the next server is busy, increment the waiting line for it
                nextServer.addToQueue();
            }else{
                if (e.getServer() == 3)
                    el.addEvent(new Event(1, clock, EventType.ARRIVAL)); //otherwise start servicing customer by creating an ARRIVAL EVENT to the next server    
                else
                    el.addEvent(new Event(2, clock, EventType.ARRIVAL)); //otherwise start servicing customer by creating an ARRIVAL EVENT to the next server
            }
        }else{
            served++;                                                       //if the current server is 2, then departure from third server means this customer finished his business here
    ...
    private void calculateInterTime(){
        if(el.eHead().getEventType()==EventType.ARRIVAL&&el.eHead().getServer()==3)
    ...
    public void printData(){                                           //prints important stats such as total time and customers served
        System.out.println("Stats for Rearranged Server Simulation: ");  
